function spikes = bz_GetMUA(varargin)
...
% do the below then filter by inputs... (Load from clu/res/fet)
...
spiketimes = res(clu==0)/samplingRate;

%% save to buzcode format (before exclusions)
if saveMat
    % save([basepath filesep sessionInfo.FileName '.MUA.cellinfo.mat'],'spikes')
    save('MUA.cellinfo.mat','spiketimes')
end
